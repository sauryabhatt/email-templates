export default function TrendDetails({res}) {
    ...
    return (
        <div dangerouslySetInnerHTML={{ __html: res.body }} style={{ paddingTop: '3%' }}></div>
    )
...
export async function getServerSideProps({ req, params: { trend = "" } = {} }) {
  ...
  const response = await fetch((process.env.REACT_APP_CONTENT_URL + `/content/${trendsObj[trend]}`), {
    method: "GET",
    headers: {
