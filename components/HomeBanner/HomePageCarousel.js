...
export default function HomePageCarousel(props){
    ...
    const imgs = [{
        ...
        hading_text: <div style={{color: "#754222"}}>Global wholesale{isMobile ?<br/>: null} buying, {isMobile ?<br/>: null}reimagined.</div>,
        small_text: <div><div>Shop wholesale products online from {isMobile ?<br/>: null}India & South East Asia.</div> <div style={{marginBottom: isMobile ? "15px" : "20px"}}>Unique products. Ethically crafted.{isMobile ?<br/>: null} Quality certified. Door delivered.</div><div>Verified buyers across 40+ countries.</div><div>Backed by a Fortune 100 company.</div></div>,
        video_link: "", 
        isSignupbtn: true,
    ...
    function change(){
        setBefore(!before)
        document.querySelector(".home-signup-btn").style.setProperty('bottom', '30%')
        let video = document.querySelectorAll("video")
        for (let i = 0; i<2;i++){
            video[i].pause()
            video[i].currentTime = 2;
        }
        if(document.querySelectorAll(".home-banner-video-reset") && document.querySelectorAll(".home-banner-video-reset")[0])
    ...
    return(
        <div className = "home-banner-carousel">
            ...
            <div className = "scroller-container">
                {!isMobile ? <span onClick = {previous} className = "previous-btn">prev</span> : null}
                {imgCount+1}/{imgs.length}
                {!isMobile ? <span onClick = {next} className = "next-btn">next</span> : null}
            </div>
        </div>
    )
}
...
function SlideElement (props){
    ...
    let {e, isVideoPlay, index, imgCount, before, isMobile, isAuthenticated } = props;
    ...
    function showVideo(flag){
        if(flag === "play"){
            document.querySelector(".home-signup-btn").style.setProperty('bottom', '60px') 
        }else{
            document.querySelector(".home-signup-btn").style.setProperty('bottom', '30%')
            setStop(true)
        }
    ...
    return (
        <div onClick={!e.isSignupbtn ? signupAction : ""} style = {isMobile ? {} : {backgroundImage: `url(${isMobile ? e.mobileImg : e.img})`}} key={index} className={`home-banner-carousel-wrap ${!e.isSignupbtn ? "pointer" : ""} ${!isVideo ? "active-banner" : ""}`}>
            {isMobile ? <img style = {{width: '100%', maxHeight: "100vh"}} alt = "" src={isMobile ? e.mobileImg : e.img}/> : null}
            ...
            {e.video_link && isVideo 
                ?  (<span className = "video-action-btn home-banner-video-reset" onClick = {()=>showVideo("stop")}>{cross()}</span>)
                : null
            }
            {e.video_link 
            ...
            {e.hading_text  
                ?  (<div style={!e.video_link ? {top: "17%"} : isVideo ? {fontSize: "30px", top: "27px"} : {}} className="home-banner-head-text">{e.hading_text}</div>)
                : null
            }
            {e.video_link && !isVideo
                ?( <span ref = {plaBtnEl} className={`home-banner-playvideo `} onClick={()=>showVideo("play")}>
            ...
            {e.small_text && !isVideo ? <div className="home-banner-small-text">{e.small_text}</div> : null}

            {e.video_link && isVideo 
                ? ( <div className="video-action-wrap">
                    <span className="video-action-btn" onClick = {isVideoPause ? ()=>playControlVideo("play") : playControlVideo}>
                        {isVideoPause ? play() : pause()}
                    </span>
                    <span className="video-action-btn" onClick = {muteVideo}>{!isSound ? volume() : mute()}</span>
                </div>)
                : null
            }
        </div>
