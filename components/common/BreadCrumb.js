...
function BreadCrumb(props) {
  ...
  let linkTo = "/products/" + encodeURIComponent("All Categories");
  if (pageId === "seller-listing") {
    pageName = "Sellers";
    linkTo = "/sellers/" + encodeURIComponent("All Categories");
  ...
  return (
    <Breadcrumb className="breadcrumb-text">
      ...
      pageId === "seller-product-listing" ? (
        <Breadcrumb.Item>
          <Link href={`/seller/${vanityId}`}><a>{brandNameSC}</a></Link>
        </Breadcrumb.Item>
      ) : (
        <Breadcrumb.Item>
          <Link href={linkTo}><a>{pageName}</a></Link>
        </Breadcrumb.Item>
      )}
      {pageId === "product-description" ? (
