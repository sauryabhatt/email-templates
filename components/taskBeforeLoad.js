...
import { setTokenLoading, setTokenSuccess, setTokenFail } from '../store/actions';
...
export const getToken = (Component) => {
    ...
    const oldToken = (localStorage['persist:appToken'] && JSON.parse(localStorage['persist:appToken']).token && JSON.parse(JSON.parse(localStorage['persist:appToken']).token)) || {};
    if((oldToken.requestedTimestamp && oldToken.expires_in && (oldToken.requestedTimestamp + oldToken.expires_in - timeSkew) <= Math.floor(Date.now()/1000)) || !(oldToken.requestedTimestamp && oldToken.expires_in)){
        ...
        return fetch((process.env.REACT_APP_KEYCLOAK_URL + "realms/" + process.env.REACT_APP_KEYCLOAK_REALM + "/protocol/openid-connect/token"), {
            method: "POST",
            body: formBody,
            headers: {
            ...
            .catch((error) => {
                ...
                if(values.redirectURI){
                    router.push('/error?message="Somthing went wrong on loading application token."&redirectURI='+values.redirectURI);
                } else {
                    router.push('/error?message="Somthing went wrong on loading application token."&redirectURI='+router.pathname);
                }
            })
    } else{
